...
var characters = [
  ...
  },
  {
    ...
    image: "public/char/pinkjoy.png"   // ← ピンク画像
  }
];
...
if (!root) {
  ...
  for (var i = 0; i < characters.length; i++) {
    ...
    // クリック時（クロージャで chara を固定）
    card.addEventListener(
      "click",
      (function (c) {
        return function () {
          localStorage.setItem("osewa_selectedCharacter", JSON.stringify(c));
          alert("「" + c.name + "」をえらびました！");
        };
      })(chara)
    );
